...
#     # FUTURE: Download scan in JSON via API v2.
#     # xml_output = qgc2.request('scan/?action=fetch&scan_ref=%s&output_format=json' % (scans[scan_title].pop()))
#     # with open("output2.xml", "w") as text_file:
#     #     text_file.write(xml_output)
#     #
#     # Parse scheduled scans for each scan reference.
#     scans_data[scan_title] = []
#     for scan_ref in scans[scan_title]:
#         scans_data[scan_title].append(defaultdict(str))
#         i = len(scans_data[scan_title])-1
#         # Store scan_ref.
#         scans_data[scan_title][i]['scan_ref'] = scan_ref
...
#         # Store XML.
#         # Slugify filename.
#         scan_filename = ''.join(c for c in scan_ref if c in valid_chars)
#         scan_filename = 'scans/' + scan_filename + '.xml'
#         with open(scan_filename, 'w') as text_file:
#             text_file.write(xml_output)
...
#         scans_data[scan_title][i]['scan_time'] = scan_time
#         # Parse individual IPs.
#         scans_data[scan_title][i]['ips'] = defaultdict(lambda : defaultdict(str))
#         scans_data[scan_title][i]['ips'] = scan_report_ips(scan_root)
#         # Flatten list of IPs.
#         # FUTURE: What do we lose by flattening while processing?
#         for ip in scans_data[scan_title][i]['ips']:
#             ips[ip].append(scans_data[scan_title][i]['ips'][ip]['duration'])
# logger.debug('ips = ')
# logger.debug(ips)
# # # Flatten list of IPs.
# # ips = defaultdict(list)
# # # FUTURE: What do we lose by flattening while processing?
# # for scan_title in scans_data:
# #     for scan_ref in scans_data[scan_title]:
# #         for ip in scan_ref.ips:
# #             ips[ip].add(ip['duration'])
# # Write scan duration data to CSV.
# with open('ips.csv', 'wb') as csvfile:
#     csvwriter = csv.writer(csvfile)
#     csvwriter.writerow(['Scan title', 'Host', 'Duration 1', 'Duration 2'])#, 'New host', 'Host disappeared', '>5% delta', '>10% delta', '>15% delta'])
#     for scan_title in scans:
#         for scan in scans_data[scan_title]:
#             for ip in scans_data[scan_title][scan]['ips']:
#                 row = [scan_title, ip]
#                 row.extend(ips[ip])
#
